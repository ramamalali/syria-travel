...
function ContactUs() {
  ...
  const [contactInfo, setContactInfo] = useState({
    ...
    description: { ar: "نحن هنا للإجابة على استفساراتكم وتلقي ملاحظاتكم على مدار الساعة.", en: "We are here to answer your inquiries and receive your feedback around the clock." },
    address: { ar: "دمشق، شارع البرامكة، بناء شركة سوا، سوريا.", en: "Damascus, Baramkeh Street, Sawa Company Building, Syria." },
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setFormData({
          ...
          phone: "",
          email: "",
          inquiryType: currentLang === 'en' ? "Booking Inquiry" : "استفسار عن حجز",
          message: ""
        });
      }
    } catch (error) {
      console.error("حدث خطأ أثناء إرسال الرسالة إلى السيرفر:", error);
      alert(currentLang === 'en' ? "Sorry, failed to send your message. Please check server connection." : "عذراً، فشل إرسال الرسالة. يرجى التحقق من اتصالك بالسيرفر.");
    }
  };

  return (
    <>
      <section className="py-24 bg-surface-container" id="contact" dir={currentLang === 'ar' ? 'rtl' : 'ltr'}>
            ...
            <div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-4">
                  <span className="material-symbols-outlined text-primary text-2xl shrink-0">location_on</span>
                  <div>
                    <h6 className="font-bold text-sm text-primary mb-0.5">
                      {currentLang === 'en' ? "Main Office" : "المكتب الرئيسي"}
                    </h6>
                    <p className="text-on-surface-variant text-xs">{getLocalizedText(contactInfo.address)}</p>
                  </div>
                </div>
                
                {/* الأرقام المجلوبة ديناميكياً */}
                <div className="flex items-start gap-4">
                  <span className="material-symbols-outlined text-primary text-2xl shrink-0">phone_in_talk</span>
                  <div>
                    <h6 className="font-bold text-sm text-primary mb-0.5">
                      {currentLang === 'en' ? "Customer Service" : "خدمة العملاء"}
                    </h6>
                    {contactInfo.phones.map((phone, index) => (
                      <p key={index} className="text-on-surface-variant text-xs" dir="ltr">{phone}</p>
                    ))}
                  </div>
                </div>
                ...
                {/* البريد الإلكتروني */}
                <div className="flex items-start gap-4">
                  <span className="material-symbols-outlined text-primary text-2xl shrink-0">mail</span>
                  <div>
                    <h6 className="font-bold text-sm text-primary mb-0.5">
                      {currentLang === 'en' ? "Email Address" : "البريد الإلكتروني"}
                    </h6>
                    <p className="text-on-surface-variant text-xs">{contactInfo.email}</p>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      placeholder={currentLang === 'en' ? "Enter your name" : "أدخل اسمك"} type="text"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="font-semibold text-xs text-primary">
                    ...
                    <input 
                      ...
                      placeholder="09XX XXX XXX" type="tel" dir="ltr"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full p-4 rounded-xl border border-outline-variant/60 bg-surface-container-low text-xs focus:ring-2 focus:ring-primary outline-none text-left" 
                    placeholder="example@mail.com" type="email" dir="ltr"
                  />
                </div>
                    ...
                    >
                      {currentLang === 'en' ? (
                        <>
                          ...
                          <option value="Complaint or Suggestion">Complaint or Suggestion</option>
                        ...
                        <>
                          ...
                          <option value="رحلات المجموعات">رحلات المجموعات</option>
                          <option value="أخرى">أخرى</option>
                        </>
                      )}
                    </select>
                    <span className={`material-symbols-outlined absolute top-4 ${currentLang === 'ar' ? 'left-4' : 'right-4'} text-gray-400 pointer-events-none text-md`}>
                      unfold_more
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    className="w-full p-4 rounded-xl border border-outline-variant/60 bg-surface-container-low text-xs focus:ring-2 focus:ring-primary outline-none" 
                    placeholder={currentLang === 'en' ? "How can we help you?" : "كيف يمكننا مساعدتك؟"} rows="4"
                  ></textarea>
                </div>

                <button type="submit" className="cursor-pointer w-full py-4 bg-primary text-on-primary rounded-xl font-bold text-sm shadow-md hover:bg-primary/90 active:scale-[0.98] transition-all">
      ...
      {showPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" dir={currentLang === 'ar' ? 'rtl' : 'ltr'}>
          <div className="bg-white p-8 rounded-3xl max-w-sm w-full text-center shadow-2xl border border-outline-variant/30 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">
              {currentLang === 'en' ? "Sent Successfully!" : "تم الإرسال بنجاح!"}
            </h3>
            ...
            <button onClick={() => setShowPopup(false)} className="w-full py-3 bg-primary text-on-primary rounded-xl font-bold text-xs hover:bg-primary/90 transition-all cursor-pointer">
