...
function Testimonials() {
  ...
  if (loading) return null; // أو يمكنك رندرة Spinner بسيط هنا
            ...
            {reviews.map((item) => (
              <SwiperSlide key={item.id}>
                <div className="bg-surface p-8 rounded-2xl border border-outline-variant relative h-full flex flex-col justify-between min-h-[320px] shadow-sm hover:shadow-md transition-shadow bg-white selection:bg-transparent">
                  <span className="material-symbols-outlined text-secondary-fixed-dim absolute top-4 left-4 text-5xl opacity-20 pointer-events-none">
                    format_quote
                  </span>
